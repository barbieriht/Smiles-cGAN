...
class smiles_coder:
    def __init__(self):
        ...
        if os.path.exists(f"unconditional_generated_files/{VOCAB_OPT}_{TOKENIZER}_smiles_vocab.npz"):
            self.load(f"unconditional_generated_files/{VOCAB_OPT}_{TOKENIZER}_smiles_vocab.npz")
...
def get_hot_smiles(file_name):
    with open(file_name, "r") as f:
        smiles = [i.strip() for i in f.readlines()]
        f.close()

    if TOKENIZER == "ATOM":
        processed_molecules, smiles_max_length = preprocessing_data(smiles, replace_dict)
    else:
        processed_molecules, smiles_max_length = preprocessing_data(smiles)

    ######## TURNING TO ONE HOT ########
    coder = smiles_coder()
    if not os.path.exists(f'unconditional_generated_files/{VOCAB_OPT}_{TOKENIZER}_smiles_vocab.npz'):
        coder.fit(processed_molecules, smiles_max_length)
        coder.save(f'unconditional_generated_files/{VOCAB_OPT}_{TOKENIZER}_smiles_vocab.npz')
    smiles_hot_arrays = coder.transform(processed_molecules)

    return smiles_hot_arrays, smiles, coder

class DrugLikeMolecules(Dataset):
    def __init__(self, transform=None, file_path="chebi_smiles_unconditional.txt"):
        self.transform = transform
        self.smiles, self.dataset, self.coder = get_hot_smiles(file_path)
...
class Generator(nn.Module):
    def __init__(self, smiles_nodes, smiles_shape, noise_dim, MIN_DIM):
        super().__init__()
        ...
        self.encoder = nn.Sequential(
            nn.Linear(smiles_nodes, MIN_DIM),
            nn.LeakyReLU(0.2),
            nn.Linear(MIN_DIM, noise_dim),
            nn.LeakyReLU(0.2)
            )
        
        self.mean_layer = nn.Linear(noise_dim, 2)
        self.logvar_layer = nn.Linear(noise_dim, 2)
        
        self.decoder = nn.Sequential(
            nn.Linear(2, noise_dim),
            nn.LeakyReLU(0.2),
            nn.Linear(noise_dim, MIN_DIM),
            nn.LeakyReLU(0.2),
            nn.Linear(MIN_DIM, smiles_nodes),
            nn.Sigmoid()
            )
...
class Discriminator(nn.Module):
    def __init__(self, smiles_nodes, smiles_shape, MIN_DIM):
        super().__init__()
        self.smiles_nodes = smiles_nodes
        self.smiles_shape = smiles_shape

        self.smile_input = nn.Sequential(
            spectral_norm(nn.Conv1d(self.smiles_nodes, MIN_DIM, kernel_size=1)),
    ...
    def forward(self, x):
        ...
        x0 = self.smile_input(x.unsqueeze(-1).to(torch.float).to(device))
...
def generator_train_step(batch_size, discriminator, generator, g_optimizer, criterion, real_smiles):
    g_optimizer.zero_grad()

    fake_smiles, mean, logvar = generator(real_smiles)
    ...
    g_loss =  g_discriminator_loss + untranslatable_loss + g_repeated_loss + vae_loss
...
def discriminator_train_step(batch_size, discriminator, generator, d_optimizer, criterion, real_smiles):
    ...
    fake_smiles, _, _ = generator(real_smiles)
...
def save_state(generator, discriminator, g_optimizer, d_optimizer,
    ...
    if epoch % save_model_in == 0 and force_break == False or force_save == True:    
        ...
        sample_smiles, _, _ = generator(sample_smiles)
        # Translating smiles

        processed_molecules = translate_smiles(sample_smiles, dataset)
        
        if not os.path.exists(f"{FOLDER_PATH}/smiles"):
            os.mkdir(f"{FOLDER_PATH}/smiles")
...
def train(generator, discriminator, criterion, batch_size=None, num_epochs = 1000, display_step = 10):
...
if __name__ == "__main__":
    ...
    params = {
              ...
              "dataset":['unconditional_chebi_selected.txt']
            }
    ...
    for pc in params_combinations:    
        ...
        NOISE_DIM = selected_params["noise_dim"]
        ...
        FOLDER_PATH = f"unconditional_generated_files/{TOKENIZER}/{VOCAB_OPT}/{MIN_DIM}/{GEN_OPT_STR}/lr{LR}_gen{GLRM}_bs{BPE}"
        ...
        generator = Generator(dataset.smiles_nodes, dataset.smiles.shape, NOISE_DIM, MIN_DIM).to(device)
        discriminator = Discriminator(dataset.smiles_nodes, dataset.smiles.shape, MIN_DIM).to(device)

        data_loader = torch.utils.data.DataLoader(dataset, BPE, shuffle=True)
        train(generator, discriminator, criterion, batch_size=BPE, num_epochs=500)    
