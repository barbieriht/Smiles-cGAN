...
from keras.layers import BatchNormalization, Flatten, LeakyReLU
...
def constroi_gerador(learning_rate):
    ...
    gen = Dense(512, activation="LeakyReLU")(joined_representation)
    gen = BatchNormalization()(gen)

    gen = Dense(512, activation="LeakyReLU")(gen)
    gen = BatchNormalization()(gen)

    smile = Dense(smile_nodes, activation='tanh')(gen)
...
def constroi_discriminador(learning_rate):
    ...
    big_hidden_layer1 = Dense(256, activation="LeakyReLU")(smile_input)
    ...
    fe = Flatten()(concatenated_output)

    dropout = Dropout(0.4)(fe)

    # transformer2 = TransformerEncoder(64, 8)(dropout)

    # out_layer = Dense(1, activation="sigmoid", name='output_layer')(transformer2)
    out_layer = Dense(1, activation="sigmoid", name='output_layer')(dropout)
...
def treinamento(gerador,discriminador,gan,dataset,latent_dim,
                n_epocas=100, bat_per_epoca=24, n_batch=128, save_interval=10):
    ...
    for epoca in range(n_epocas):
        ...
        for batch in range(bat_per_epoca):
            ...
            d_loss_real = discriminador.train_on_batch([X_real, labels_real], y_real)
            ...
            d_loss_fake = discriminador.train_on_batch([X_fake,labels_fake], y_fake)
...
if __name__ == "__main__":
    ...
    params = {"learning_rate": [0.0002, 0.001],
              "bat_per_epoca": [16, 32, 64]}
    ...
    for param in combinations:
        ...
        gan = define_gan(gerador,discriminador,learning_rate=this_params["learning_rate"])

        statistics = treinamento(gerador,discriminador,gan,[smiles_arrays, classes_arrays],latent_dim, bat_per_epoca=this_params["bat_per_epoca"], n_epocas=5000)
    
        plot(statistics, bat_per_epoca=this_params["bat_per_epoca"], learning_rate=this_params["learning_rate"])